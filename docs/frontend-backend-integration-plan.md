...
- 插件保存会上传云端，网页端软删除会安全下行到插件本地回收站，插件本地删除/恢复也已能通过持久化队列上行；网页恢复下行、重命名同步和多端并发冲突处理仍待补齐。
...
2026-08-01 生产手工验收已通过：插件登录后连续本地保存、数量即时刷新、后台云端上传、失败队列持久化、生产 API CORS、Web 聚焦刷新、网页软删除下行，以及插件本地删除/恢复上行。测试文件夹和 `4` 篇文案的云端数量、回收站数量与插件状态全程一致。尚未通过：网页恢复下行、重命名、多端并发冲突，以及第二台真实设备上的项目深链接恢复。
...
2. 基于已完成的“网页软删除下行”和“插件本地删除/恢复上行”，补齐网页恢复下行、重命名和冲突处理，避免恢复后仍留在本地回收站或旧名称覆盖新名称。
