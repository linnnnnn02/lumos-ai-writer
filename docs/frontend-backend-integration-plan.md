...
- 插件保存会上传云端，网页端软删除也已能安全下行到插件本地回收站；插件本地删除上行、网页恢复下行、重命名同步和并发冲突处理仍待补齐。
...
当前生产状态：API、插件 sidepanel 保存链路和 Web 文案库详情页均已接入真实云端数据。2026-07-31 使用真实账号连续保存 `4` 篇素材，插件数量逐篇即时增加，网页重新聚焦后由 `1` 个文件夹、`4` 篇文案自动刷新为 `2` 个文件夹、`8` 篇文案；新增文件夹、标题和片段一致。云端删除的测试文件夹及 `4` 篇文案已在插件重新加载后自动进入本地可恢复回收站，正常文件夹和未匹配本地数据保持不变。尚未覆盖插件本地删除上行、网页恢复下行、重命名和并发冲突处理。
...
2026-07-31 生产手工验收已通过：插件登录后连续本地保存、数量即时刷新、后台云端上传、失败队列持久化、生产 API CORS、Web 聚焦刷新，以及网页软删除到插件本地可恢复回收站的一致性。尚未通过：跨端恢复、重命名、本地删除上行、并发冲突，以及第二台真实设备上的项目深链接恢复。
...
2. 基于已完成的“网页软删除下行”和稳定云端 ID，补齐插件本地删除/恢复上行、网页恢复下行、重命名和冲突处理，避免恢复后仍留在本地回收站或旧名称覆盖新名称。
