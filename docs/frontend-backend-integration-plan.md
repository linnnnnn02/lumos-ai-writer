...
- 插件 sidepanel 已接入生产可用的 MVP 云端同步：同一账号邮箱密码登录；保存标注时先原子写入 `chrome.storage.local` 并立即刷新数量，再交由持久化后台队列调用幂等同步 API。失败任务可重试，保存交互不再等待网络。
...
- 插件已实现第一阶段云端删除下行：登录后会自动续期会话并读取云端回收站，按精确 ID 或去查询参数的来源链接，将可靠匹配的本地文件夹/文案移入可恢复回收站；不会永久删除，也不会用空云端快照覆盖本地数据。真实账号已完成 `1` 个文件夹、`4` 篇文案和片段的跨端软删除验收。
- 插件本地记录已支持持久化稳定的云端 ID：新标注上传成功后立即记录服务端身份，已有数据会按唯一来源链接和文件夹归属安全回填；存在重复链接或跨文件夹歧义时不会猜测绑定，为本地删除/恢复上行队列提供可靠基础。
- 插件本地删除/恢复已接入持久化云端操作队列：本地回收站依然即时反馈，后台异步同步云端；断网任务保留并在启动、登录或刷新后重试，同一资源的新操作会替换旧操作，且不允许跨账号执行。真实账号已通过文件夹及 `4` 篇文案的“恢复 -> 云端出库 -> 再删除 -> 云端入回收站”完整验收。
- 网页恢复已能安全下行到插件：插件刷新云端快照时，会将云端已恢复的文件夹、文案和片段从本地回收站恢复，并回填最新云端 ID 和文件夹名称。本地待删除/待恢复操作会暂时优先于旧云端快照，避免断网或请求延迟时出现“刚删除又恢复”或“刚恢复又删除”。
- 文件夹和文案名称已接入双向同步：网页端改名会在插件刷新云端快照时下行；插件管理页支持内联重命名并通过持久化操作队列异步上行。待重命名任务与待上传标注会暂时保护本地名称，避免旧云端快照反向覆盖。
...
- Cloudflare Pages 生产站已发布至 `https://lumos-ai-writer.pages.dev`；真实账号已通过插件连续采集 `4` 篇素材、网页聚焦自动刷新和项目深链接冷启动验收。
...
现在最大的问题已经从“工程数据没打通”转为：**生产 MVP 的单向上传闭环已打通，但跨端数据生命周期、正式插件分发和 AI 质量验收仍未收口。**

具体表现：

- 真实账号、云端文案库、项目持久化和 DeepSeek 接口已经跑通，生产站由 Cloudflare Pages 自动部署。
- 插件构建产物可以由 GitHub Actions 生成，生产 CORS 已支持实际 Chrome 扩展来源；正式商店分发、版本升级和回滚流程仍待建立。
- 插件保存会上传云端，网页端软删除/恢复与重命名会安全下行到插件，插件本地删除/恢复/重命名也已能通过持久化队列上行；多端并发冲突处理仍待补齐。
- 旧插件本地数据的批量迁移尚未完成，目前只保证新保存标注的增量幂等上传。
- AI 已记录 token 和运行状态；成本估算支持可配置 token 单价，但线上质量试用和硬性预算拦截尚未完成。
...
5. 管理页读取云端数据。网页端已完成；插件管理页仍以本地库为准。
6. 本地缓存作为 fallback。保存链路已完成，完整云端回拉尚未完成。
7. 增加同步队列和重试提示。已完成持久化后台队列、失败状态和手动重试。
...
当前生产状态：API、插件 sidepanel 保存链路和 Web 文案库详情页均已接入真实云端数据。2026-07-31 使用真实账号连续保存 `4` 篇素材，插件数量逐篇即时增加，网页重新聚焦后由 `1` 个文件夹、`4` 篇文案自动刷新为 `2` 个文件夹、`8` 篇文案；新增文件夹、标题和片段一致。云端删除的测试文件夹及 `4` 篇文案已在插件重新加载后自动进入本地可恢复回收站，正常文件夹和未匹配本地数据保持不变。2026-08-01 已完成插件本地恢复/删除上行和网页恢复下行验收：网页恢复后云端从 `1` 文件夹 / `4` 文案变为 `2` 文件夹 / `8` 文案且回收站归零，插件本地自动出现“验收-可删除 4”且回收站归零；再从网页删除后云端回到 `1` 文件夹 / `4` 文案，回收站恢复为 `5` 项。重命名双向同步已完成代码与自动化验证，待合并部署后做真实账号往返验收；多端并发冲突处理仍待补齐。
...
2026-08-01 生产手工验收已通过：插件登录后连续本地保存、数量即时刷新、后台云端上传、失败队列持久化、生产 API CORS、Web 聚焦刷新、网页软删除/恢复下行，以及插件本地删除/恢复上行。测试文件夹和 `4` 篇文案的云端数量、回收站数量与插件状态全程一致。重命名双向同步已完成代码与自动化验证，待部署后补真实账号往返验收。尚未通过：多端并发冲突，以及第二台真实设备上的项目深链接恢复。
...
1. 在第二台真实设备上打开生产项目深链接，验收项目、对话和阶段的跨设备恢复。
2. 在已完成的删除/恢复/重命名双向同步上建立明确的多端冲突策略，避免离线旧操作覆盖较新的名称或不同设备反复回滚。
3. 实现旧插件本地数据的首次批量迁移和重复上传去重。
4. 后续完善正式插件设备码绑定、设备列表、撤销登录和版本升级渠道。
5. 在预算开关和单用户额度保护下开启小范围真实 AI 质量试用，继续记录 `ai_runs`，再决定是否扩大调用。
6. 拆分网页主包和低频页面代码，降低首次加载成本，并补充对应性能基线。
