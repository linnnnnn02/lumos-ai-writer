...
- 插件保存会上传云端，但网页端的删除、恢复和重命名还不会反向同步到插件本地库，需要先确定数据源和冲突策略。
...
当前生产状态：API、插件 sidepanel 保存链路和 Web 文案库详情页均已接入真实云端数据。2026-07-31 使用真实账号连续保存 `4` 篇素材，插件数量逐篇即时增加，网页重新聚焦后由 `1` 个文件夹、`4` 篇文案自动刷新为 `2` 个文件夹、`8` 篇文案；新增文件夹、标题和片段一致。测试云端文件夹已移入回收站。当前仍是插件向云端的增量上传，网页删除与恢复不会自动反向更新插件本地库。
...
2026-07-31 生产手工验收已通过：插件登录后连续本地保存、数量即时刷新、后台云端上传、失败队列持久化、生产 API CORS 和 Web 聚焦刷新。尚未通过：网页与插件的跨端删除/恢复一致性，以及第二台真实设备上的项目深链接恢复。
...
2. 明确插件本地库与云端库的数据源和冲突规则，补齐删除、恢复、重命名的跨端一致性，避免已删除素材被下一次保存重新创建。
