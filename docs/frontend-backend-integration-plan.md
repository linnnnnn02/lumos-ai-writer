...
- 插件保存会上传云端，网页端软删除/恢复会安全下行到插件，插件本地删除/恢复也已能通过持久化队列上行；重命名同步和多端并发冲突处理仍待补齐。
...
2026-08-01 生产手工验收已通过：插件登录后连续本地保存、数量即时刷新、后台云端上传、失败队列持久化、生产 API CORS、Web 聚焦刷新、网页软删除/恢复下行，以及插件本地删除/恢复上行。测试文件夹和 `4` 篇文案的云端数量、回收站数量与插件状态全程一致。尚未通过：重命名、多端并发冲突，以及第二台真实设备上的项目深链接恢复。
...
2. 基于已完成的删除/恢复双向同步，补齐文件夹和文案重命名同步，再建立明确的多端冲突策略，避免旧名称覆盖新名称或不同设备反复回滚。
