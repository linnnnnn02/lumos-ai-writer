...
Task Skills such as `reference-analysis`, `xiaohongshu-draft`, `selection-rewrite`, and `target-reader-preview` are consumers of that model. They must not independently invent a user style or promote a one-off project instruction into a long-term preference.
